...
const buildJsonUrl = (crateName: string, version?: string, target?: string, formatVersion?: number): string => {
  ...
  const fetchCrateJsonWithStatus = async (
    ...
    try {
      ...
      if (!data || typeof data !== 'object') {
        throw new JSONParseError(JSON.stringify(data), new Error('Response is not a valid object'), url);
      }
