        ...
        resources: [
          {
            ...
            uri: "cache://config",
            description: "Get current server configuration (cache TTL, max size, DB path, etc.)"
          }
        ]
      })
    }),
  ...
  const start = async (): Promise<void> => {
    try {
      if (config.useStdio) {
        ...
        ErrorLogger.logInfo("MCP Rust Docs Server is running (stdio)", { config })
      } else {
        // HTTP transport
              ...
              if (sessionId && transports[sessionId]) {
                ...
                transport = new WebStandardStreamableHTTPServerTransport({
                  ...
                  onsessioninitialized: (id) => {
                    ...
                    ErrorLogger.logInfo("Session initialized", { sessionId: id })
                  },
                  onsessionclosed: (id) => {
                    delete transports[id]
                    ErrorLogger.logInfo("Session closed", { sessionId: id })
                  }
                })
        ...
        ErrorLogger.logInfo("MCP Rust Docs Server is running (HTTP)", {
          config,
