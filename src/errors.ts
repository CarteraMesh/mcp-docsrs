...
export class CacheError extends MCPDocsRsError {
  ...
  constructor(operation: 'get' | 'set' | 'delete' | 'clear' | 'close' | 'query' | 'stats' | 'list', details?: string) {
...
export const ErrorLogger = {
    ...
    if (process.env.NODE_ENV === 'test' || process.env.BUN_ENV === 'test') {
      ...
      if (
        ...
        if (process.env.LOG_EXPECTED_ERRORS === 'true') {
          ...
          console.error(`\x1b[32m[EXPECTED ERROR] ${error.name}: ${error.message}\x1b[0m`);
        } else {
          // Brief indicator in green to show test is working correctly
          console.error(`\x1b[32m✓ Expected ${error.name} thrown\x1b[0m`);
        }
        return;
      }
    }
  ...
  logWarning(message: string, context?: Record<string, unknown>): void {
    ...
    console.error(`[${timestamp}] WARNING: ${message}${contextStr}`);
  },
