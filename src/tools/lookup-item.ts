import { z } from 'zod';
import { ErrorLogger, ItemNotFoundError, isCrateNotFoundError, isJSONParseError, isMCPDocsRsError } from '../errors.js';
...
export const createLookupItemHandler = (fetcher: DocsFetcher) => {
  return async (args: LookupItemArgs): Promise<DocsResponse> => {
    try {
      const { data: json, fromCache } = await fetcher.fetchCrateJson(args.crateName, args.version, args.target);
      ...
      if (isJSONParseError(error)) {
        errorMessage = 'Failed to parse JSON from docs.rs. The response may not be valid rustdoc JSON.';
...
export const lookupItemPromptSchema = {
  ...
  itemPath: z.string().optional().describe('Path to specific item (e.g., "struct.MyStruct" or "fn.my_function")'),
  version: z.string().optional().describe('Specific version or semver range'),
  target: z.string().optional().describe('Target platform'),
};
