...
const cacheTtl = Number.parseInt((values['cache-ttl'] as string) || process.env.CACHE_TTL || '3600000');
const maxCacheSize = Number.parseInt((values['max-cache-size'] as string) || process.env.MAX_CACHE_SIZE || '100');
const requestTimeout = Number.parseInt((values['request-timeout'] as string) || process.env.REQUEST_TIMEOUT || '30000');
