...
export const createCache = <T>(maxSize = 100, dbPath?: string) => {
  ...
  const remove = (key: string): void => {
    try {
      ...
      ErrorLogger.log(error as Error)
      throw new CacheError("delete", `Failed to delete key '${key}': ${(error as Error).message}`)
    }
  }
  ...
  const getStats = (): { totalEntries: number; totalSize: number; oldestEntry: Date | null } => {
    try {
      const count = (countStmt.get() as { count: number }).count
      const sizeResult = db.prepare("SELECT SUM(LENGTH(data)) as totalSize FROM cache").get() as {
      ...
      ErrorLogger.log(error as Error)
      throw new CacheError("stats", `Failed to get cache statistics: ${(error as Error).message}`)
    }
  }
  ...
  const close = (): void => {
    try {
      ...
      ErrorLogger.log(error as Error)
      throw new CacheError("close", `Failed to close cache database: ${(error as Error).message}`)
    }
  }
